...
void searchByStatus() {
    ...
    printf("Nhap trang thai cua Todo can tim kiem (1: Hoan thanh, 0: Chua hoan thanh): ");
    scanf("%d", &searchStatus);
    
    printf("Danh sach Todo theo trang thai %s:\n", searchStatus ? "Hoan thanh" : "Chua hoan thanh");
    for (int i = 0; i < todoCount; i++) {
        if (todoList[i].status == searchStatus) {
